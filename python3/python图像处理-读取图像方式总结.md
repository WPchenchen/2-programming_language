...
```
                ...
                with open(image_path, 'rb') as f:
                    data_img = f.read()
                # 转换成 NumPy 数组
                image_array = np.frombuffer(data_img, np.uint8)
                # 解码成 OpenCV 图像
...
```
 save_path = os.path.join(output_folder, f'{backend1}_2_{i}.jpg')
 cv2.imencode(".jpg", result[0][0][0])[1].tofile(save_path)
```

###### 内存方式（不直接写文件）

如果你想把图像保存到 **内存中的字节流**（比如传给 socket 或 web API），可以用 `cv2.imencode`。

```
ret, buf = cv2.imencode(".jpg", img, [cv2.IMWRITE_JPEG_QUALITY, 90])
if ret:
    byte_data = buf.tobytes()
    # 可以写入 socket、HTTP 响应、数据库
```
...
```
import numpy as np
img_recover = cv2.imdecode(np.frombuffer(byte_data, np.uint8), cv2.IMREAD_COLOR)
```
